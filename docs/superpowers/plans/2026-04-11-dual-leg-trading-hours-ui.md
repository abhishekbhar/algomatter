# Dual-Leg & Trading Hours Strategy Form UI — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add collapsible Trading Hours and Dual-Leg Execution panels to the strategy create and edit pages so users can configure `rules.trading_hours` and `rules.dual_leg` without a DB console.

**Architecture:** Two self-contained page file edits — `new/page.tsx` first (simpler, no data loading), then `edit/page.tsx` (adds useEffect unpacking). Both pages use plain `useState`, Chakra UI's `Collapse` component for the collapsible panels, and extend the rules payload on submit with conditional spreading.

**Tech Stack:** Next.js 14 (App Router), Chakra UI v2, TypeScript, plain `useState` (no form library)
...
| File | Change |
|------|--------|
| `frontend/app/(dashboard)/strategies/new/page.tsx` | Extend form state, add panel state, add two collapsible panels, update submit payload and validation |
| `frontend/app/(dashboard)/strategies/[id]/edit/page.tsx` | Same panels, update useEffect to unpack new rule fields, update submit payload and validation |

---

## Task 1: Update new/page.tsx
...
```bash
cd frontend && npx tsc --noEmit 2>&1 | grep -E "new/page|error" | head -20
```
...
git commit -m "feat: add trading hours and dual-leg panels to new strategy page"
```

---

## Task 2: Update edit/page.tsx

**Files:**
- Modify: `frontend/app/(dashboard)/strategies/[id]/edit/page.tsx`
...
```bash
cd frontend && npx tsc --noEmit 2>&1 | grep -E "edit/page|error" | head -20
```
...
```bash
git add frontend/app/\(dashboard\)/strategies/\[id\]/edit/page.tsx
git commit -m "feat: add trading hours and dual-leg panels to edit strategy page"
```
...
- [ ] **Step 1: Rsync frontend to server**
...
Expected last lines: something like `Route (app) ... ○ (Static)` with no `Error` lines.

- [ ] **Step 3: Re-apply CSP patch and restart**
...
SSHPASS="$SERVER_PASS" nix-shell -p sshpass --run \
  ...
  systemctl restart algomatter-frontend"'
```
...
SSHPASS="$SERVER_PASS" nix-shell -p sshpass --run \
  'sshpass -e ssh -o StrictHostKeyChecking=no root@194.61.31.226 \
  "sleep 5 && curl -s -o /dev/null -w \"App: %{http_code}\n\" https://algomatter.in/app"'
```
...
```bash
git add docs/superpowers/plans/2026-04-11-dual-leg-trading-hours-ui.md
git commit -m "docs: add implementation plan for dual-leg trading hours UI"
```
